...
export function AuthProvider({ children }) {
  ...
  const toUser = async (session) => {
    ...
    // NOTE: profiles has no avatar_url column (confirmed against the actual
    // table schema) — selecting it caused PostgREST to 400 on every single
    // call, silently falling back to meta on every load. Only select columns
    // that actually exist.
