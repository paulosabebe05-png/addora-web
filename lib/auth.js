...
export function AuthProvider({ children }) {
  ...
  /* ─────────────────────────────────────────
     Phone + OTP (via AfroMessage, verified server-side)
  ───────────────────────────────────────── */
  const sendPhoneOtp = async (phone) => {
    ...
    return data // { success: true, phone: '+2519xxxxxxxx' }
  }
  ...
  const verifyPhoneOtp = async (phone, code, name) => {
    ...
    // The server route verified the OTP with AfroMessage and created/signed
    // in the Supabase user, returning real session tokens. We apply that
    // session to the browser client here so useAuth() picks it up.
