...
export default function CartPage() {
  ...
  if (!user) {
    return (
      <div className={styles.guestWrap}>
        ...
        <h2>{tr('signInToViewCart')}</h2>
        <p>{tr('cartSignInSub')}</p>
        <Link href="/auth/signin?redirect=/cart" className={styles.signInBtn}>{tr('signIn')}</Link>
      </div>
    )
  }
  ...
  if (items.length === 0) {
    return (
      <div className={styles.guestWrap}>
        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" opacity="0.25"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
        <h2>{tr('cartEmptyTitle')}</h2>
        <p>{tr('cartEmptySub')}</p>
        <Link href="/" className={styles.signInBtn}>{tr('browseProducts')}</Link>
      </div>
    )
  }
  ...
  return (
    <div className={styles.page}>
      <h1 className={styles.title}>{tr('myCartTitle')}</h1>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.summary}>
          ...
          <div className={styles.summaryRow}>
            <span>{tr('subtotal')} ({items.reduce((s, i) => s + i.qty, 0)} {tr('items')})</span>
            <span>ETB {total.toLocaleString()}</span>
          </div>
          <div className={styles.summaryRow}>
            <span>{tr('deliveryFee')}</span>
            <span className={styles.green}>{tr('atCheckout')}</span>
          </div>
          ...
          <div className={styles.codNote}>
            ...
            {tr('payWithCOD')} <strong style={{ marginLeft: 3 }}>{tr('cashOnDeliveryFull')}</strong>
          </div>
